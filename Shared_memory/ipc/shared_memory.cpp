...
namespace Ipc
{
    ...
    uint16_t SharedMemory::AppendToTheEnd(const std::vector<uint8_t>& data)
    {
        ...
        std::cout<<"End of appending"<<std::endl;
        return index;
    }
    ...
    {
        UpdateShmProperties();
        std::cout<<"Read Mem " <<(m_beginPtr + BytesForCurrentSizeValue).getPtr()<<" "<<m_currentEndPtr.getPtr()<<std::endl;
    ...
    {
        std::vector<uint8_t> tmpData ((m_beginPtr + BytesForCurrentSizeValue).getPtr(), (m_beginPtr + BytesForLastIndexValue).getPtr());
    ...
    {
        if (shm_unlink(m_memoryName.c_str()) == errorValue)
        {
