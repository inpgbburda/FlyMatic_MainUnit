...
static Address MakeAddress(const Parameters& param)
{
    ...
    tmpAddr = tmpAddr & param.modId;
    tmpAddr = tmpAddr << sizeof(param.label) & param.label;
    tmpAddr = tmpAddr << sizeof(param.priority) & param.priority;
    tmpAddr = tmpAddr << sizeof(param.option) & param.option;

    return tmpAddr;
}

static Address MakeAddressCopy(const Address& address)
{
    return MakeAddress({ .modId = GetModId(address), .label = GetLabel(address), .priority = GetPriority(address), .option = GetOption(address) });
}
